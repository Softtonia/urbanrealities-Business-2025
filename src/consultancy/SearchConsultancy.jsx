...
function SearchConsultancy() {
    ...
    const handleSubmit = async (id, userStatus) => {
        try {
            ...
            const response = await post('/api/send-request-by-company-to-consultancy', { consultancy_id: id });
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                const response = await get(`/search-consultancy-by-id`);
    ...
    return (
        <div className='consultancy-search-bar'>
            <input className='' type='text' placeholder='Find consultancy by id' value={input} onChange={handleChange} />
            {load && filteredData.length > 0 && (
                <div className='consultancy-search-data'>
...
export default SearchConsultancy;
