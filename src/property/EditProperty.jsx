...
function EditProperty() {
  ...
  useEffect(() => {
    console.log(id);
    get(`/api/get-data-properties/${id}`)
      .then((response) => {
        // setFormData({
        //   ...formData,
        ...
        setFormData2({
          ...
          property_type_id: propertyType?.map(val => val.property_type_id) || [],
          property_status_id: propertyStatus?.map(val => val.property_status_id) || [],
          purpose_id,
          name: property_name,
          street_address,
          country_id,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formData.append("project", formData2.project);
  ...
  useEffect(() => {
    post(
      `/api/get-all-project-by-location-id`,
      {
                ...
                <div className="col-lg-4 col-md-4 col-sm-12 add-listing-form-col">
                  ...
                  <div className="custom-select">
                    <Select
                      ...
                      value={Array.isArray(projectName) && projectName.find((dev) => dev.id === formData2.project)
                        ? {
                          value: formData2.project,
                          label: projectName.find((dev) => dev.id === formData2.project).name,
                        }
                        : null
                      }
                      onChange={(selectedOption) =>
                        setFormData2((prev) => ({
                          ...prev,
                          project: selectedOption ? selectedOption.value : "", // Only save the ID
                        }))
                      }
                      ...
                      {record.options.map((option, index) => (
                        <label key={index} className="radio-label">
                          <input
                            ...
                            value={option.value}
                            checked={option.value === record.field_value} // ✅ Controlled input
                            onChange={(e) =>
                              handleChange(e, record.id || record.custom_field_id, record.field_type)
                            }
                          />
