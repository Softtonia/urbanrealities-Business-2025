  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await post(
        `/api/add-properties-listing`, formData,);

      setProgress(false);
      toast.success("Data Inserted Successfully!");
      nav("/my-account/all-listing");
    } catch (error) {
      setProgress(false);
  ...
  useEffect(() => {
    let fetchPurposeId = async () => {
      await post(
        ...
        { model_fields: formData2.model_fields, post_type: "property_list" }
      )
        .then((response) => {
      ...
      <div className=" add-property-listing-search-wraper">
        <div className=" add-property-form-wraper">
          <div className="add-property-form-h-wraper">
            <h1 className="m-0">Add Property Listing</h1>
          </div>
          {
            // form start here
          }
