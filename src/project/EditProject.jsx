...
// import { Select, MenuItem, FormControl,Checkbox, InputLabel ,TextField,} from '@mui/material';
...
// import "../All-property-listing/AddFormListing.css";
// import axios from "axios";
// import { API_KEY, VITE_TINY_API_KEY } from "../../src/config";
// import Navbar from "../Layout/Navbar";
// import { toast } from "react-toastify";
// import Select from "react-select";
// import { Editor } from "@tinymce/tinymce-react";
// import { MyContext } from "../Store/AuthContext";
// import MediaUpload from "../All-property-listing/MediaUpload";
...
// import MediaUploadEdit from "../All-property-listing/MediaUploadEdit";
...
import { Editor } from "@tinymce/tinymce-react";
// import { MyContext } from "../Store/AuthContext";
import MediaUpload from "./media-and-Image-upload/MediaUpload";
import Accordion from "@mui/material/Accordion";
import AccordionSummary from "@mui/material/AccordionSummary";
import AccordionDetails from "@mui/material/AccordionDetails";
import ExpandMoreIcon from "@mui/icons-material/ExpandMore";
...
import FeaturedImageUpload from "../components/FeaturedImageUpload/FeaturedImageUpload";
import { useNavigate, useParams } from "react-router-dom/dist";
import MediaUploadEdit from "./media-and-Image-upload/MediaUploadEdit";
import { useDebounce } from "../hook/useDebounce";
import SearchableDropdown from "../components/searchabledropdown/SearchableDropdown";
...
function EditProject() {
  ...
  const getOrCreateKeyword = async (keyword) => {
    try {
      const res = await get(`/api/get-keyword-by-keyword-type?keyword_type=project_keyword&search=${encodeURIComponent(keyword)}`);
  ...
  const fetchKeywords = async (query) => {
    ...
    try {
      const res = await get(`/api/get-keyword-by-keyword-type?keyword_type=project_keyword&search=${encodeURIComponent(query)}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formData.append("developer", formData2.developer);
      ...
      const response = post(
        `/api/edit-project-listing`,
        formData
      );

      setProgress(false);
      toast.success("Data Inserted Successfully!");
      nav('/my-account/all-project')
    } catch (error) {
      setProgress(false);
  ...
  console.log("developer", formData2.developer)
  ...
  useEffect(() => {
    post(
      `/api/get-all-developer-by-location-id`,
      {
        country_id: formData2.country_id,
        state_id: formData2.state_id,
        city_id: formData2.city_id,
      }
    )
      .then((response) => {
        console.log(response);
  ...
  }, [formData2.country_id, formData2.state_id, formData2.city_id]);
  useEffect(() => {
    get(`/api/fetch-all-developer-listing-by-admin`)
      .then((response) => {
        console.log(response);
        setProjectName(response.data);
      })
  ...
  useEffect(() => {
    if (fetchedKeyword && fetchedKeyword.length > 0) {
      ...
      setKeywordObjects(fetchedKeyword.map(k => ({         // map to your expected object keys
        id: k.id,
        keyword_name: k.keyword_name  // or just name depending on your UI usage
      })));
    }
  }, [fetchedKeyword]);
      ...
      <div className=" add-property-listing-search-wraper">
        <div className=" add-property-form-wraper">
          <div className="add-property-form-h-wraper">
            <h1 className="m-0">Edit Project Listing</h1>
          </div>
          {
            // form start here
          }
              ...
              {formData2.property_id ? (
                <div className="col-lg-4 col-md-4 col-sm-12 add-listing-form-col">
                  ...
                  <div className="custom-select">
                    <Select
                      name="developer"
                      options={Array.isArray(projectName) && projectName.map((record) => ({
                        value: record.id,
                        label: record.name,
                      }))}
                      value={Array.isArray(projectName) && projectName.find((dev) => dev.id === formData2.developer)
                        ? {
                          value: formData2.developer,
                          label: projectName.find((dev) => dev.id === formData2.developer).name,
                        }
                        : null
                      }
                      onChange={(selectedOption) =>
                        setFormData2((prev) => ({
                          ...prev,
                          developer: selectedOption ? selectedOption.value : "", // Only save the ID
                        }))
                      }
                  ...
                  {errors.project_error && (
                    <p className="errors-msg">{errors.project_error}</p>
                  )}
                </div>
              ) : <div className="col-lg-4 col-md-4 col-sm-12 add-listing-form-col"></div>}
